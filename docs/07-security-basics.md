...
```bash
kubectl -n hello exec deploy/hello-api -- id
```
...
kubectl -n hello exec deploy/hello-api -- sh -lc 'touch /etc/should-fail'
# expect: Read-only file system

# 3) /tmp is writable
kubectl -n hello exec deploy/hello-api -- sh -lc 'echo ok >/tmp/probe && cat /tmp/probe'
# expect: ok
```
